...
def find_latest_summary_files_by_filename_timestamp(date_folder_path: str) -> dict:
    """
    For each study subfolder within a date folder, find the latest CSV file for each category
            ...
            try:
                ...
                for file in files:
                    ...
                    try:
                        ...
                        logger.warning(f"Could not parse timestamp '{timestamp_str}' from file {file_name}: {e}")
                        continue
                ...
                for category, info in latest_in_study.items():
                    if info:
                        summary_files[category].append((info['path'], info['name']))
                        logger.info(f"Latest {category} → {info['name']} (timestamp: {info['datetime']})")
